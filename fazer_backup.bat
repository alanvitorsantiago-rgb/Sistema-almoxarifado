...
echo =========================================
echo    INICIANDO SCRIPT DE BACKUP DO BANCO
echo =========================================
...
if not exist "%BACKUP_DIR%" (
    ...
    if errorlevel 1 (
        echo.
        echo X ERRO: Nao foi possivel criar a pasta de backups.
        echo    Por favor, crie a pasta 'backups' manualmente na raiz do projeto.
...
if not exist "%DB_FILE%" (
    echo.
    echo X ERRO: Arquivo do banco de dados (database.db) nao encontrado.
    echo    Verifique se voce esta executando o script na pasta raiz do projeto.
    echo O script sera encerrado.
    pause
    exit /b 1
)
...
if not defined DATETIME_STR (
    echo.
    echo X ERRO: Nao foi possivel obter a data e hora do sistema via WMIC.
    echo O script sera encerrado.
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo.
    echo X ERRO: Falha ao copiar o arquivo do banco de dados.
